...
class TaladroCarguioScreen extends StatefulWidget {
  ...
  const TaladroCarguioScreen({Key? key, this.rolUsuario, this.dniUsuario})
    : super(key: key);
...
class _TaladroCarguioScreenState extends State<TaladroCarguioScreen> {
  ...
  Future<void> _cargarEstadosOperacion() async {
    ...
    List<Map<String, dynamic>> estados = await DatabaseHelper()
        .getEstadosByOperacionIdCarguio(operacionId!);
                  ...
                  ElevatedButton(
                    onPressed: () async {
                      ...
                      int filasAfectadas = await DatabaseHelper()
                          .eliminarOperacionTalCarguioFisico(operacionId!);
  ...
  Future<void> _actualizarRegistroEstado(
    ...
    try {
      ...
      if (actualizado) {
        ...
        todosLosEstados = await DatabaseHelper().getEstadosByOperacionIdCarguio(
          operacionActual!['id'],
        );
  ...
  Future<List<Map<String, dynamic>>> _getOperaciones() async {
    if (operacionActual == null) return [];
    return await DatabaseHelper().getEstadosByOperacionIdCarguio(
      operacionActual!['id'],
    );
  }
  ...
  void _mostrarDialogoSecundario(int estadoId, String estado) async {
    ...
    Future.delayed(Duration.zero, () {
      if (estado == "OPERATIVO") {
        _abrirDialogoOperativoCarguio(estadoId);
      } else {
        _abrirDialogoNoOperativoCarguio(estadoId, estado);
      }
    });
  }

  Future<void> _abrirDialogoOperativoCarguio(int estadoId) async {
  ...
  Future<void> _handleNuevaOperacion(Map<String, dynamic> data) async {
    ...
    List<Map<String, dynamic>> horometros = await dbHelper
        .getHorometrosPorOperacion('carguio');
    ...
    List<Map<String, dynamic>> checklistItems = await DatabaseHelper()
        .getCheckListByProceso('SCOOPTRAM');
  ...
  Future<void> _editarOperacionOperativoCarguio(
    ...
    Map<String, dynamic> datos = await DatabaseHelper()
        .getOperacionByEstadoIdCarguio(operacionActual!['id'], operacion['id']);
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return DialogoHorometro(
          ...
          onSave: (id, map) =>
              DatabaseHelper().updateHorometrosCarguio(id, map),
          ...
          onConfirmar: () async {
            ...
            try {
              ...
              List<Map<String, dynamic>> todosLosEstados =
                  await DatabaseHelper().getEstadosByOperacionIdCarguio(
                    operacionActual!['id'],
                  );
              ...
              await DatabaseHelper().cerrarOperacionCarguio(
                operacionActual!['id'],
              );
