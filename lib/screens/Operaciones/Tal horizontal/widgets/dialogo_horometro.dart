...
class _DialogoHorometroState extends State<DialogoHorometro> {
  ...
  setState(() {
    // Convertir el mapa de horómetros a la lista que usa el diálogo
    horometros = [
      {
        'id': 1,
        'nombre': 'Diesel',
  ...
  // Validar un horómetro específico
String? _validarHorometro(Map<String, dynamic> horometro, int index) {
  ...
  // Validación INOP: ambos valores deben estar vacíos (null o 0)
  if (estaINOP) {
    if ((inicial != 0)) {
  ...
  // Construir el mapa de horómetros para guardar
  ...
  if (guardado) {
    ...
    Navigator.pop(context, true); // Devolver true indicando éxito
  } else {
    _mostrarSnackbar(
  ...
  // Determinar si es un caso especial (final igual a inicial)
