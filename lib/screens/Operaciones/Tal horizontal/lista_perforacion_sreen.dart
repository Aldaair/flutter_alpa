...
class TaladroHorizontalScreen extends StatefulWidget {
  final String? rolUsuario;
  final String? dniUsuario;

  const TaladroHorizontalScreen({Key? key, this.rolUsuario, this.dniUsuario}) : super(key: key);

  @override
  State<TaladroHorizontalScreen> createState() => _TaladroHorizontalScreenState();
}

class _TaladroHorizontalScreenState extends State<TaladroHorizontalScreen> {
...
Future<void> _fetchOperacionData() async {
  ...
  if (widget.rolUsuario == 'Master') {
    data = await dbHelper.getOperacionTalHorizontalByTurnoAndFechaMaster(
        selectedTurno!, fechaActual);
  } else {
    data = await dbHelper.getOperacionTalHorizontalByTurnoAndFecha(
        selectedTurno!, fechaActual);
  }
...
Future<void> _cargarEstadosOperacion() async {
  ...
  List<Map<String, dynamic>> estados =
      await DatabaseHelper().getEstadosByOperacionIdHorizontal(operacionId!);
              ...
              children: [
                ...
                ElevatedButton(
                  onPressed: () async {
                    ...
                    int filasAfectadas =
                        await DatabaseHelper().eliminarOperacionTalHorizontalFisico(operacionId!);
...
void _mostrarDialogoEstado(String estado) async {
  ...
  List<Map<String, dynamic>> todosLosEstados = await DatabaseHelper()
      .getEstadosByOperacionIdHorizontal(operacionActual!['id']);
...
Future<void> _crearRegistroEstado(Map<String, dynamic> data, String estado) async {
  try {
    ...
    if (ultimoEstadoActivo != null) {
      await DatabaseHelper().updateHoraFinalHorizontal(
        operacionActual!['id'],
    ...
    Map<String, dynamic>? nuevoEstado = await DatabaseHelper().createEstadoHorizontal(
      operacionActual!['id'],
      estado,
      data['codigo']!,
      data['hora_inicio']!,
...
Future<void> _actualizarRegistroEstado(Map<String, dynamic> data, Map<String, dynamic> operacionOriginal) async {
  try {
    ...
    bool actualizado = await DatabaseHelper().updateEstadoHorizontal(
      operacionActual!['id'],
      data['id'],
      numero: data['numero'],
      estado: data['estado'],
...
Future<List<Map<String, dynamic>>> _getOperaciones() async {
  if (operacionActual == null) return [];
  return await DatabaseHelper().getEstadosByOperacionIdHorizontal(operacionActual!['id']);
}
...
Future<void> _abrirDialogoPerforacion(int estadoId) async {
  Map<String, dynamic> datosPerforacion = await DatabaseHelper()
      .getOperacionByEstadoIdHorizontal(operacionActual!['id'], estadoId);
    ...
    builder: (BuildContext context) {
      return DialogoFormularioPerforacion(
        ...
        onGuardar: (datosActualizados) async {
          bool guardado = await DatabaseHelper().updateOperacionByEstadoIdHorizontal(
            operacionActual!['id'],
            estadoId,
            datosActualizados,
          );
...
void _abrirDialogoNoOperativo(int estadoId, String estado) {
  // 👇 AQUÍ INVOCAS TU DIÁLOGO ESPECÍFICO
  showDialog(
    context: context,
    builder: (BuildContext context) {
      return DialogoFormularioNoOpePerforacion(
        operacionId: operacionActual!['id'],
        estadoId: estadoId,
        estado: estado, // "DEMORA", "MANTENIMIENTO", etc.
        primaryColor: primaryColor,
        onGuardar: (datosActualizados) async {
          bool guardado = await DatabaseHelper().updateOperacionByEstadoIdHorizontal(
            operacionActual!['id'],
...
Future<void> _handleNuevaOperacion(Map<String, dynamic> data) async {
  ...
  List<Map<String, dynamic>> horometros =
      await dbHelper.getHorometrosPorOperacion('tal_horizontal');
...
Future<void> _editarOperacionOperativo(Map<String, dynamic> operacion) async {
  Map<String, dynamic> datosPerforacion = await DatabaseHelper()
      .getOperacionByEstadoIdHorizontal(operacionActual!['id'], operacion['id']);
    ...
    builder: (BuildContext context) {
      return DialogoFormularioPerforacion(
        ...
        onGuardar: (datosActualizados) async {
          ...
          if (guardado) {
            _mostrarSnackBar("Datos de perforación actualizados", Colors.green);
            await _fetchOperacionData();
          } else {
...
Future<void> _editarOperacionNoOperativo(Map<String, dynamic> operacion, String estado) async {
        ...
        onConfirmar: () async {
          ...
          try {
            var ultimoEstado = await DatabaseHelper()
                .getUltimoEstadoByOperacionIdHorizontal(operacionActual!['id']);
            ...
            bool actualizado = await DatabaseHelper().updateHoraFinalHorizontal(
              operacionActual!['id'],
              ultimoEstado['id'],
              horaReservaInicio,
            );
            ...
            await DatabaseHelper().createReservaEstadoHorizontal(
              operacionActual!['id'],
              newNumber,
              horaReservaInicio,
              horaReservaFinal,
            ...
            await DatabaseHelper().cerrarOperacionHorizontal(operacionActual!['id']);
...
void _handleCondicionesEquipo() async {
  ...
  Map<String, dynamic> condicionesData = await DatabaseHelper()
      .getCondicionesEquipoByOperacionIdHorizontal(operacionId);
...
void _handlePresionLlantas() async {
  ...
  Map<String, dynamic> controlLlantas = await DatabaseHelper()
      .getControlLlantasByOperacionIdHorizontal(operacionId);
...
Future<void> _eliminarRegistroEstado(Map<String, dynamic> estado) async {
  try {
    ...
    bool eliminado = await DatabaseHelper().deleteEstadoHorizontal(
      operacionActual!['id'],
      estado['id'],
    );
