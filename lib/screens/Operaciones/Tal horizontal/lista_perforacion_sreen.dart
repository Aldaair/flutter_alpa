...
class TaladroHorizontalScreen extends StatefulWidget {
  ...
  const TaladroHorizontalScreen({Key? key, this.rolUsuario, this.dniUsuario})
    : super(key: key);

  @override
  State<TaladroHorizontalScreen> createState() =>
      _TaladroHorizontalScreenState();
}
...
class _TaladroHorizontalScreenState extends State<TaladroHorizontalScreen> {
  ...
  Future<void> _cargarEstadosOperacion() async {
    ...
    List<Map<String, dynamic>> estados = await DatabaseHelper()
        .getEstadosByOperacionIdHorizontal(operacionId!);
                  ...
                  ElevatedButton(
                    onPressed: () async {
                      ...
                      int filasAfectadas = await DatabaseHelper()
                          .eliminarOperacionTalHorizontalFisico(operacionId!);
  ...
  Future<List<Map<String, dynamic>>> _getOperaciones() async {
    if (operacionActual == null) return [];
    return await DatabaseHelper().getEstadosByOperacionIdHorizontal(
      operacionActual!['id'],
    );
  }
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return DialogoFormularioPerforacion(
          ...
          estado: "OPERATIVO",
          fecha: operacionActual!['fecha']?.toString() ?? fechaActual,
          turno: operacionActual!['turno']?.toString() ?? selectedTurno ?? '',
          primaryColor: primaryColor,
          onGuardar: (datosActualizados) async {
            bool guardado = await DatabaseHelper()
                .updateOperacionByEstadoIdHorizontal(
                  operacionActual!['id'],
                  estadoId,
                  datosActualizados,
                );
  ...
  Future<void> _handleNuevaOperacion(Map<String, dynamic> data) async {
    ...
    if (createPlan.isBlocked) {
      _mostrarSnackBar(createPlan.blockingMessage!, Colors.orange);
      return;
    }
  ...
  Future<void> _editarOperacionOperativo(Map<String, dynamic> operacion) async {
    Map<String, dynamic> datosPerforacion = await DatabaseHelper()
        .getOperacionByEstadoIdHorizontal(
          operacionActual!['id'],
          operacion['id'],
        );
          ...
          onConfirmar: () async {
            ...
            try {
              var ultimoEstado = await DatabaseHelper()
                  .getUltimoEstadoByOperacionIdHorizontal(
                    operacionActual!['id'],
                  );
              ...
              List<Map<String, dynamic>> todosLosEstados =
                  await DatabaseHelper().getEstadosByOperacionIdHorizontal(
                    operacionActual!['id'],
                  );
              ...
              await DatabaseHelper().cerrarOperacionHorizontal(
                operacionActual!['id'],
              );
