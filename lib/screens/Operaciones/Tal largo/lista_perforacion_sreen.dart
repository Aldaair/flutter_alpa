...
class TaladroLargoScreen extends StatefulWidget {
  final String? rolUsuario;
  final String? dniUsuario;

  const TaladroLargoScreen({Key? key, this.rolUsuario, this.dniUsuario})
    : super(key: key);

  @override
  State<TaladroLargoScreen> createState() => _TaladroLargoScreenState();
}

class _TaladroLargoScreenState extends State<TaladroLargoScreen> {
  ...
  Future<void> _fetchOperacionData() async {
    ...
    if (_isMaster) {
      if (selectedOperatorId != null) {
        data = await dbHelper.getOperacionTalLargoByTurnoAndFecha(
          selectedTurno!,
          fechaActual,
          operadorId: selectedOperatorId,
        );
      } else {
        data = await dbHelper.getOperacionTalLargoByTurnoAndFechaMaster(
          selectedTurno!,
          fechaActual,
        );
      }
                  ...
                  ElevatedButton(
                    onPressed: () async {
                      ...
                      int filasAfectadas = await DatabaseHelper()
                          .eliminarOperacionTalLargoFisico(operacionId!);
  ...
  void _mostrarDialogoEstado(String estado) async {
    ...
    // 🔥 ORDEN CORRECTO: usar NUMERO (NO HORA)
  ...
  Future<void> _crearRegistroEstado(
    ...
    try {
      ...
      if (ultimoEstadoActivo != null) {
        await DatabaseHelper().updateHoraFinal(
          operacionActual!['id'],
  ...
  Future<void> _actualizarRegistroEstado(
    ...
    try {
      ...
      bool actualizado = await DatabaseHelper().updateEstado(
        operacionActual!['id'],
        data['id'],
        numero: data['numero'],
        estado: data['estado'],
        ...
        return DialogoConfirmarCierreRegistros(
          ...
          onConfirmar: () async {
            ...
            try {
              ...
              bool actualizado = await DatabaseHelper().updateHoraFinal(
                operacionActual!['id'],
                ultimoEstado['id'],
                horaReservaInicio,
              );
              ...
              List<Map<String, dynamic>> todosLosEstados =
                  await DatabaseHelper().getEstadosByOperacionId(
                    operacionActual!['id'],
                  );
  ...
  Future<void> _eliminarRegistroEstado(Map<String, dynamic> estado) async {
    try {
      ...
      bool eliminado = await DatabaseHelper().deleteEstado(
        operacionActual!['id'],
        estado['id'],
      );
