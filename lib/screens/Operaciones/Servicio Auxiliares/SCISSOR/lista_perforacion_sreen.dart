...
import 'package:i_miner/screens/Operaciones/Servicio%20Auxiliares/SCISSOR/widgets/dialogo_horometro.dart';
import 'package:i_miner/screens/Operaciones/Servicio%20Auxiliares/SCISSOR/widgets/dialogo_no_operativo_formulario_perforacion.dart';
...
class TaladroSCISSORScreen extends StatefulWidget {
  final String? rolUsuario;
  final String? dniUsuario;

  const TaladroSCISSORScreen({Key? key, this.rolUsuario, this.dniUsuario})
    : super(key: key);

  @override
  State<TaladroSCISSORScreen> createState() => _TaladroSCISSORScreenState();
}

class _TaladroSCISSORScreenState extends State<TaladroSCISSORScreen> {
  ...
  Future<void> _fetchOperacionData() async {
    ...
    if (_isMaster) {
      if (selectedOperatorId != null) {
        data = await dbHelper.getOperacionScissorByTurnoAndFecha(
          selectedTurno!,
          fechaActual,
          operadorId: selectedOperatorId,
        );
      } else {
        data = await dbHelper.getOperacionScissorByTurnoAndFechaMaster(
          selectedTurno!,
          fechaActual,
        );
      }
  ...
  Future<void> _cargarEstadosOperacion() async {
    ...
    List<Map<String, dynamic>> estados = await DatabaseHelper()
        .getEstadosByOperacionIdScissor(operacionId!);
                  ...
                  ElevatedButton(
                    onPressed: () async {
                      ...
                      int filasAfectadas = await DatabaseHelper()
                          .eliminarOperacionTalScissorFisico(operacionId!);
  ...
  void _mostrarDialogoEstado(String estado) async {
    ...
    List<Map<String, dynamic>> todosLosEstados = await DatabaseHelper()
        .getEstadosByOperacionIdScissor(operacionActual!['id']);
  ...
  Future<void> _crearRegistroEstado(
    ...
    try {
      ...
      if (ultimoEstadoActivo != null) {
        await DatabaseHelper().updateHoraFinalScissor(
          operacionActual!['id'],
      ...
      Map<String, dynamic> operacionData = {
        ...
        // Campos de DESTINO
        'destino_nivel': data['destino_nivel'] ?? '',
        'destino_tipo_labor': data['destino_tipo_labor'] ?? '',
        'destino_labor': data['destino_labor'] ?? '',
        'destino_ala': data['destino_ala'] ?? '',
  ...
  Future<void> _actualizarRegistroEstado(
    ...
    try {
      ...
      bool actualizado = await DatabaseHelper().updateEstadoScissor(
        operacionActual!['id'],
        data['id'],
        numero: data['numero'],
        estado: data['estado'],
      ...
      if (actualizado) {
        ...
        todosLosEstados = await DatabaseHelper().getEstadosByOperacionIdScissor(
          operacionActual!['id'],
        );
  ...
  Future<List<Map<String, dynamic>>> _getOperaciones() async {
    if (operacionActual == null) return [];
    return await DatabaseHelper().getEstadosByOperacionIdScissor(
      operacionActual!['id'],
    );
  }
  ...
  void _mostrarDialogoSecundario(int estadoId, String estado) async {
    ...
    Future.delayed(Duration.zero, () {
      if (estado == "OPERATIVO") {
        _abrirDialogoOperativoScissor(estadoId);
      } else {
        _abrirDialogoNoOperativoScissor(estadoId, estado);
      }
    });
  }

  Future<void> _abrirDialogoOperativoScissor(int estadoId) async {
    Map<String, dynamic> datosPerforacion = await DatabaseHelper()
        .getOperacionByEstadoIdScissor(operacionActual!['id'], estadoId);
        ...
        return DialogoFormularioRompebanco(
          ...
          onGuardar: (datosActualizados) async {
            bool guardado = await DatabaseHelper()
                .updateOperacionByEstadoIdScissor(
                  operacionActual!['id'],
                  estadoId,
                  datosActualizados,
                );
  ...
  void _abrirDialogoNoOperativoScissor(int estadoId, String estado) async {
    ...
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return DialogoFormularioNoOpeScissor(
          operacionId: operacionActual!['id'],
          estadoId: estadoId,
          estado: estado,
  ...
  Future<void> _handleNuevaOperacion(Map<String, dynamic> data) async {
    ...
    /// Insertar operación
    await dbHelper.insertOperacionScissor(
      data['fecha'],
      data['turno'],
      data['operador'],
      data['jefe_guardia'],
  ...
  void _editarOperacion(Map<String, dynamic> operacion) async {
    ...
    try {
      ...
      if (estado == "OPERATIVO") {
        await _editarOperacionOperativoScissor(operacion);
      } else {
        await _editarOperacionNoOperativoScissor(operacion, estado);
      }
    } catch (e) {
      if (Navigator.canPop(context)) Navigator.pop(context);
  ...
  Future<void> _editarOperacionOperativoScissor(
    Map<String, dynamic> operacion,
  ) async {
    Map<String, dynamic> datos = await DatabaseHelper()
        .getOperacionByEstadoIdScissor(operacionActual!['id'], operacion['id']);
  ...
  Future<void> _editarOperacionNoOperativoScissor(
    Map<String, dynamic> operacion,
    String estado,
  ) async {
    Map<String, dynamic> datos = await DatabaseHelper()
  ...
  void _handleHorometro() async {
    ...
    Map<String, dynamic> horometrosData = await DatabaseHelper()
        .getHorometrosByOperacionIdScissor(operacionId);
          ...
          onConfirmar: () async {
            ...
            try {
              var ultimoEstado = await DatabaseHelper()
                  .getUltimoEstadoByOperacionIdScissor(operacionActual!['id']);
              ...
              bool actualizado = await DatabaseHelper().updateHoraFinalScissor(
                operacionActual!['id'],
                ultimoEstado['id'],
                horaReservaInicio,
              );
              ...
              List<Map<String, dynamic>> todosLosEstados =
                  await DatabaseHelper().getEstadosByOperacionIdScissor(
                    operacionActual!['id'],
                  );
              ...
              await DatabaseHelper().createReservaEstadoScissor(
                operacionActual!['id'],
                newNumber,
                horaReservaInicio,
                horaReservaFinal,
              ...
              await DatabaseHelper().cerrarOperacionScissor(
                operacionActual!['id'],
              );
  ...
  void _handleCondicionesEquipo() async {
    ...
    Map<String, dynamic> condicionesData = await DatabaseHelper()
        .getCondicionesEquipoByOperacionIdScissor(operacionId);
  ...
  void _handlePresionLlantas() async {
    ...
    Map<String, dynamic> controlLlantas = await DatabaseHelper()
        .getControlLlantasByOperacionIdScissor(operacionId);
  ...
  Future<void> _eliminarRegistroEstado(Map<String, dynamic> estado) async {
    try {
      ...
      bool eliminado = await DatabaseHelper().deleteEstadoScissor(
        operacionActual!['id'],
        estado['id'],
      );
