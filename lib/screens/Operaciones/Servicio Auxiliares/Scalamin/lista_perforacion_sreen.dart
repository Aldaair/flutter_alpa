...
import 'package:i_miner/screens/Operaciones/Servicio%20Auxiliares/Scalamin/widgets/dialog_check_imagen.dart';
import 'package:i_miner/screens/Operaciones/Servicio%20Auxiliares/Scalamin/widgets/dialogo_checklist.dart';
import 'package:i_miner/screens/Operaciones/Servicio%20Auxiliares/Scalamin/widgets/dialogo_condiciones_equipo.dart';
import 'package:i_miner/screens/Operaciones/Servicio%20Auxiliares/Scalamin/widgets/dialogo_confirmar_cierre.dart';
...
import 'package:i_miner/screens/Operaciones/Servicio%20Auxiliares/Scalamin/widgets/dialogo_no_operativo_formulario_perforacion.dart'
...
class TaladroScalaminScreen extends StatefulWidget {
  final String? rolUsuario;
  final String? dniUsuario;

  const TaladroScalaminScreen({Key? key, this.rolUsuario, this.dniUsuario})
    : super(key: key);

  @override
  State<TaladroScalaminScreen> createState() => _TaladroScalaminScreenState();
}

class _TaladroScalaminScreenState extends State<TaladroScalaminScreen> {
  ...
  void obtenerEstadosBD() async {
    estadosBD = await DatabaseHelper().getEstadosBD('SCALAMIN');
  ...
  Future<void> _fetchOperacionData() async {
    ...
    if (_isMaster) {
      if (selectedOperatorId != null) {
        data = await dbHelper.getOperacionScalaminByTurnoAndFecha(
          selectedTurno!,
          fechaActual,
          operadorId: selectedOperatorId,
        );
      } else {
        data = await dbHelper.getOperacionScalaminByTurnoAndFechaMaster(
          selectedTurno!,
          fechaActual,
        );
      }
  ...
  Future<void> _cargarEstadosOperacion() async {
    ...
    List<Map<String, dynamic>> estados = await DatabaseHelper()
        .getEstadosByOperacionIdScalamin(operacionId!);
                  ...
                  ElevatedButton(
                    onPressed: () async {
                      ...
                      int filasAfectadas = await DatabaseHelper()
                          .eliminarOperacionTalScalaminFisico(operacionId!);
  ...
  void _mostrarDialogoEstado(String estado) async {
    ...
    List<Map<String, dynamic>> todosLosEstados = await DatabaseHelper()
        .getEstadosByOperacionIdScalamin(operacionActual!['id']);
  ...
  Future<void> _crearRegistroEstado(
    ...
    try {
      ...
      if (ultimoEstadoActivo != null) {
        await DatabaseHelper().updateHoraFinalScalamin(
          operacionActual!['id'],
  ...
  Future<void> _actualizarRegistroEstado(
    ...
    try {
      ...
      bool actualizado = await DatabaseHelper().updateEstadoScalamin(
        operacionActual!['id'],
        data['id'],
        numero: data['numero'],
        estado: data['estado'],
  ...
  Future<List<Map<String, dynamic>>> _getOperaciones() async {
    if (operacionActual == null) return [];
    return await DatabaseHelper().getEstadosByOperacionIdScalamin(
      operacionActual!['id'],
    );
  }
  ...
  Future<void> _abrirDialogoRompebanco(int estadoId) async {
    Map<String, dynamic> datosRompebanco = await DatabaseHelper()
        .getOperacionByEstadoIdScalamin(operacionActual!['id'], estadoId);
        ...
        return DialogoFormularioRompebanco(
          ...
          onGuardar: (datosActualizados) async {
            bool guardado = await DatabaseHelper()
                .updateOperacionByEstadoIdScalamin(
                  operacionActual!['id'],
                  estadoId,
                  datosActualizados,
                );
  ...
  Future<void> _handleNuevaOperacion(Map<String, dynamic> data) async {
    ...
    // 🔥 1. OBTENER HORÓMETROS DE SCALAMIN
    List<Map<String, dynamic>> horometros = await dbHelper
        .getHorometrosPorOperacion('scalamin');

    print("✅ Horómetros scalamin:");
    ...
    List<Map<String, dynamic>> checklistItems = await DatabaseHelper()
        .getCheckListByProceso('SCALAMIN');
    ...
    /// Insertar operación
    await dbHelper.insertOperacionScalamin(
      data['fecha'],
      data['turno'],
      data['operador'],
      data['jefe_guardia'],
  ...
  Future<void> _editarOperacionOperativo(Map<String, dynamic> operacion) async {
    Map<String, dynamic> datosRompebanco = await DatabaseHelper()
        .getOperacionByEstadoIdScalamin(
          operacionActual!['id'],
          operacion['id'],
        );
  ...
  void _handleHorometro() async {
    ...
    Map<String, dynamic> horometrosData = await DatabaseHelper()
        .getHorometrosByOperacionIdScalamin(operacionId);
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return DialogoHorometro(
          ...
          onSave: (id, map) =>
              DatabaseHelper().updateHorometrosScalamin(id, map),
          headerTitle: 'Horómetros - Scalamin',
          ...
          onConfirmar: () async {
            ...
            try {
              var ultimoEstado = await DatabaseHelper()
                  .getUltimoEstadoByOperacionIdScalamin(operacionActual!['id']);
              ...
              bool actualizado = await DatabaseHelper().updateHoraFinalScalamin(
                operacionActual!['id'],
                ultimoEstado['id'],
                horaReservaInicio,
              );
              ...
              List<Map<String, dynamic>> todosLosEstados =
                  await DatabaseHelper().getEstadosByOperacionIdScalamin(
                    operacionActual!['id'],
                  );
              ...
              await DatabaseHelper().createReservaEstadoScalamin(
                operacionActual!['id'],
                newNumber,
                horaReservaInicio,
                horaReservaFinal,
              ...
              await DatabaseHelper().cerrarOperacionScalamin(
                operacionActual!['id'],
              );
  ...
  void _handleCondicionesEquipo() async {
    ...
    Map<String, dynamic> condicionesData = await DatabaseHelper()
        .getCondicionesEquipoByOperacionIdScalamin(operacionId);
  ...
  void _handlePresionLlantas() async {
    ...
    Map<String, dynamic> controlLlantas = await DatabaseHelper()
        .getControlLlantasByOperacionIdScalamin(operacionId);
  ...
  Future<void> _eliminarRegistroEstado(Map<String, dynamic> estado) async {
    try {
      ...
      bool eliminado = await DatabaseHelper().deleteEstadoScalamin(
        operacionActual!['id'],
        estado['id'],
      );
