...
import 'package:i_miner/screens/Operaciones/Servicio%20Auxiliares/Rompebancos/widgets/dialog_check_imagen.dart';
import 'package:i_miner/screens/Operaciones/Servicio%20Auxiliares/Rompebancos/widgets/dialogo_checklist.dart';
import 'package:i_miner/screens/Operaciones/Servicio%20Auxiliares/Rompebancos/widgets/dialogo_condiciones_equipo.dart';
import 'package:i_miner/screens/Operaciones/Servicio%20Auxiliares/Rompebancos/widgets/dialogo_confirmar_cierre.dart';
...
import 'package:i_miner/screens/Operaciones/Servicio%20Auxiliares/Rompebancos/widgets/dialogo_no_operativo_formulario_perforacion.dart'
    show DialogoFormularioNoOpePerforacion;
...
class TaladroRompebancosScreen extends StatefulWidget {
  final String? rolUsuario;
  final String? dniUsuario;

  const TaladroRompebancosScreen({Key? key, this.rolUsuario, this.dniUsuario})
    : super(key: key);

  @override
  State<TaladroRompebancosScreen> createState() =>
      _TaladroRompebancosScreenState();
}

class _TaladroRompebancosScreenState extends State<TaladroRompebancosScreen> {
  ...
  void obtenerEstadosBD() async {
    estadosBD = await DatabaseHelper().getEstadosBD('ROMPEBANCOS');
  ...
  Future<void> _fetchOperacionData() async {
    ...
    if (_isMaster) {
      if (selectedOperatorId != null) {
        data = await dbHelper.getOperacionRompeBacoByTurnoAndFecha(
          selectedTurno!,
          fechaActual,
          operadorId: selectedOperatorId,
        );
      } else {
        data = await dbHelper.getOperacionRompeBacoByTurnoAndFechaMaster(
          selectedTurno!,
          fechaActual,
        );
      }
  ...
  Future<void> _cargarEstadosOperacion() async {
    ...
    List<Map<String, dynamic>> estados = await DatabaseHelper()
        .getEstadosByOperacionIdRompeBaco(operacionId!);
                  ...
                  ElevatedButton(
                    onPressed: () async {
                      ...
                      int filasAfectadas = await DatabaseHelper()
                          .eliminarOperacionTalRompeBacoFisico(operacionId!);
  ...
  void _mostrarDialogoEstado(String estado) async {
    ...
    List<Map<String, dynamic>> todosLosEstados = await DatabaseHelper()
        .getEstadosByOperacionIdRompeBaco(operacionActual!['id']);
  ...
  Future<void> _crearRegistroEstado(
    ...
    try {
      ...
      if (ultimoEstadoActivo != null) {
        await DatabaseHelper().updateHoraFinalRompeBaco(
          operacionActual!['id'],
  ...
  Future<void> _actualizarRegistroEstado(
    ...
    try {
      ...
      bool actualizado = await DatabaseHelper().updateEstadoRompeBaco(
        operacionActual!['id'],
        data['id'],
        numero: data['numero'],
        estado: data['estado'],
  ...
  Future<List<Map<String, dynamic>>> _getOperaciones() async {
    if (operacionActual == null) return [];
    return await DatabaseHelper().getEstadosByOperacionIdRompeBaco(
      operacionActual!['id'],
    );
  }
  ...
  void _mostrarDialogoSecundario(int estadoId, String estado) async {
    ...
    Future.delayed(Duration.zero, () {
      if (estado == "OPERATIVO") {
        // 📌 CASO 1: ES OPERATIVO → Formulario de Rompebanco
        _abrirDialogoRompebanco(estadoId);
      } else {
  ...
  Future<void> _abrirDialogoRompebanco(int estadoId) async {
    Map<String, dynamic> datosRompebanco = await DatabaseHelper()
        .getOperacionByEstadoIdRompeBaco(operacionActual!['id'], estadoId);
        ...
        return DialogoFormularioRompebanco(
          ...
          onGuardar: (datosActualizados) async {
            bool guardado = await DatabaseHelper()
                .updateOperacionByEstadoIdRompeBaco(
                  operacionActual!['id'],
                  estadoId,
                  datosActualizados,
                );
            ...
            if (guardado) {
              _mostrarSnackBar("Datos de Rompebanco guardados", Colors.green);
              await _fetchOperacionData();
            } else {
  ...
  Future<void> _handleNuevaOperacion(Map<String, dynamic> data) async {
    ...
    /// Insertar operación
    await dbHelper.insertOperacionRompeBaco(
      data['fecha'],
      data['turno'],
      data['operador'],
      data['jefe_guardia'],
  ...
  Future<void> _editarOperacionOperativo(Map<String, dynamic> operacion) async {
    Map<String, dynamic> datosRompebanco = await DatabaseHelper()
        .getOperacionByEstadoIdRompeBaco(
          operacionActual!['id'],
          operacion['id'],
        );
      ...
      builder: (BuildContext context) {
        return DialogoFormularioRompebanco(
          ...
          onGuardar: (datosActualizados) async {
            ...
            if (guardado) {
              _mostrarSnackBar(
                "Datos de Rompebanco actualizados",
                Colors.green,
              );
              await _fetchOperacionData();
            } else {
  ...
  void _handleChecklist() async {
    ...
    List<Map<String, dynamic>> checklistData = await DatabaseHelper()
        .getCheckListByOperacionIdRompeBaco(operacionId);
  ...
  void _handleHorometro() async {
    ...
    Map<String, dynamic> horometrosData = await DatabaseHelper()
        .getHorometrosByOperacionIdRompeBaco(operacionId);
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return DialogoHorometro(
          ...
          onSave: (id, map) =>
              DatabaseHelper().updateHorometrosRompeBaco(id, map),
          headerTitle: 'Horómetros - Rompebanco',
          headerSubtitle: 'Diesel y Percusión',
          ...
          onConfirmar: () async {
            ...
            try {
              var ultimoEstado = await DatabaseHelper()
                  .getUltimoEstadoByOperacionIdRompeBaco(
                    operacionActual!['id'],
                  );
              ...
              List<Map<String, dynamic>> todosLosEstados =
                  await DatabaseHelper().getEstadosByOperacionIdRompeBaco(
                    operacionActual!['id'],
                  );
              ...
              await DatabaseHelper().createReservaEstadoRompeBaco(
                operacionActual!['id'],
                newNumber,
                horaReservaInicio,
                horaReservaFinal,
              ...
              await DatabaseHelper().cerrarOperacionRompeBaco(
                operacionActual!['id'],
              );
  ...
  void _handleCondicionesEquipo() async {
    ...
    Map<String, dynamic> condicionesData = await DatabaseHelper()
        .getCondicionesEquipoByOperacionIdRompeBaco(operacionId);
  ...
  void _handlePresionLlantas() async {
    ...
    Map<String, dynamic> controlLlantas = await DatabaseHelper()
        .getControlLlantasByOperacionIdRompeBaco(operacionId);
  ...
  Future<void> _eliminarRegistroEstado(Map<String, dynamic> estado) async {
    try {
      ...
      bool eliminado = await DatabaseHelper().deleteEstadoRompeBaco(
        operacionActual!['id'],
        estado['id'],
      );
