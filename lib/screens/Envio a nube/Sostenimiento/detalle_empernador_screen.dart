...
class DetalleSeccionScreenEmpernador extends StatefulWidget {
  final String tipoOperacion;
  final Color primaryColor;

  const DetalleSeccionScreenEmpernador({
    Key? key,
  ...
  @override
  State<DetalleSeccionScreenEmpernador> createState() => _DetalleSeccionEmpernadorScreenState();
}

class _DetalleSeccionEmpernadorScreenState extends State<DetalleSeccionScreenEmpernador> {
  ...
  late ExportarEmpernadorService _exportarService;

  @override
  void initState() {
    super.initState();
    _dbHelper = DatabaseHelper();
    _exportarService = ExportarEmpernadorService(_dbHelper);
    _fetchOperacionData();
  }
  ...
  Future<void> _fetchOperacionData() async {
    try {
      List<Map<String, dynamic>> data = await _dbHelper
          .getOperacionesTaladroEmpernador();
  ...
  Future<void> _eliminarRegistrosSeleccionados() async {
    ...
    try {
      ...
      for (var id in selectedItems) {
        int result = await _dbHelper.eliminarOperacionTalEmpernadorFisico(id);
...
Future<void> _enviarDatosALaNube(List<Map<String, dynamic>> jsonData) async {
  ...
  try {
    ...
    /// 🔥 IMPORTANTE: tipo correcto en backend
    final success = await operacionService.crear(
      'empernador', // 👈 ESTE ES CLAVE (debe coincidir con tu backend)
      dataParaEnviar,
    );
    ...
    if (success) {
      ...
      for (var item in jsonData) {
        ...
        await _dbHelper.actualizarEnvioEmpernador(localId);
      }
