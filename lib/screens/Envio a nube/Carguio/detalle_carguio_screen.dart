...
class DetalleSeccionScreenCarguio extends StatefulWidget {
  final String tipoOperacion;
  final Color primaryColor;

  const DetalleSeccionScreenCarguio({
    Key? key,
  ...
  @override
  State<DetalleSeccionScreenCarguio> createState() => _DetalleSeccionCarguioScreenState();
}

class _DetalleSeccionCarguioScreenState extends State<DetalleSeccionScreenCarguio> {
  ...
  late ExportarCarguioService _exportarService;

  @override
  void initState() {
    super.initState();
    _dbHelper = DatabaseHelper();
    _exportarService = ExportarCarguioService(_dbHelper);
    _fetchOperacionData();
  }
  ...
  Future<void> _eliminarRegistrosSeleccionados() async {
    ...
    try {
      ...
      for (var id in selectedItems) {
        int result = await _dbHelper.eliminarOperacionTalCarguioFisico(id);
...
Future<void> _enviarDatosALaNube(List<Map<String, dynamic>> jsonData) async {
  ...
  try {
    ...
    /// 🔥 ENVÍO MASIVO (CLAVE)
    final success = await operacionService.crear('carguio', dataParaEnviar);
    ...
    if (success) {
      ...
      for (var item in jsonData) {
        ...
        await _dbHelper.actualizarEnvioCarguio(localId);
      }
