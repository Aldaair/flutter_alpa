...
class DetalleSeccionScreenDumper extends StatefulWidget {
  final String tipoOperacion;
  final Color primaryColor;

  const DetalleSeccionScreenDumper({
    Key? key,
  ...
  @override
  State<DetalleSeccionScreenDumper> createState() => _DetalleSeccionDumperScreenState();
}

class _DetalleSeccionDumperScreenState extends State<DetalleSeccionScreenDumper> {
  ...
  late ExportarDumperService _exportarService;

  @override
  void initState() {
    super.initState();
    _dbHelper = DatabaseHelper();
    _exportarService = ExportarDumperService(_dbHelper);
    _fetchOperacionData();
  }
  ...
  Future<void> _eliminarRegistrosSeleccionados() async {
    ...
    try {
      ...
      for (var id in selectedItems) {
        int result = await _dbHelper.eliminarOperacionTalDumperFisico(id);
...
Future<void> _enviarDatosALaNube(List<Map<String, dynamic>> jsonData) async {
  ...
  try {
    ...
    /// ✅ TIPO CORRECTO
    final success = await operacionService.crear('dumper', dataParaEnviar);
    ...
    if (success) {
      ...
      for (var item in jsonData) {
        ...
        await _dbHelper.actualizarEnvioDumper(localId);
      }
