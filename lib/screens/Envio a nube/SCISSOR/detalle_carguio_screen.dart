...
class DetalleSeccionScreenSCISSOR extends StatefulWidget {
  final String tipoOperacion;
  final Color primaryColor;

  const DetalleSeccionScreenSCISSOR({
    Key? key,
  ...
  @override
  State<DetalleSeccionScreenSCISSOR> createState() => _DetalleSeccionSCISSORScreenState();
}

class _DetalleSeccionSCISSORScreenState extends State<DetalleSeccionScreenSCISSOR> {
  ...
  late ExportarScissorService _exportarService;

  @override
  void initState() {
    super.initState();
    _dbHelper = DatabaseHelper();
    _exportarService = ExportarScissorService(_dbHelper);
    _fetchOperacionData();
  }
  ...
  Future<void> _eliminarRegistrosSeleccionados() async {
    ...
    try {
      ...
      for (var id in selectedItems) {
        int result = await _dbHelper.eliminarOperacionTalScissorFisico(id);
...
Future<void> _enviarDatosALaNube(List<Map<String, dynamic>> jsonData) async {
  ...
  try {
    ...
    /// 🔥 ENVÍO MASIVO (SCISSOR)
    final success = await operacionService.crear('scissor', dataParaEnviar);
    ...
    if (success) {
      ...
      for (var item in jsonData) {
        ...
        await _dbHelper.actualizarEnvioscissor(localId);
      }
