...
class DetalleSeccionScreenRompeBanco extends StatefulWidget {
  final String tipoOperacion;
  final Color primaryColor;

  const DetalleSeccionScreenRompeBanco({
    Key? key,
  ...
  @override
  State<DetalleSeccionScreenRompeBanco> createState() => _DetalleSeccionRompeBancoScreenState();
}

class _DetalleSeccionRompeBancoScreenState extends State<DetalleSeccionScreenRompeBanco> {
  ...
  late ExportarRompebancoService _exportarService;

  @override
  void initState() {
    super.initState();
    _dbHelper = DatabaseHelper();
    _exportarService = ExportarRompebancoService(_dbHelper);
    _fetchOperacionData();
  }
  ...
  Future<void> _fetchOperacionData() async {
    try {
      List<Map<String, dynamic>> data = await _dbHelper
          .getOperacionesTaladroRompeBaco();
  ...
  Future<void> _eliminarRegistrosSeleccionados() async {
    ...
    try {
      ...
      for (var id in selectedItems) {
        int result = await _dbHelper.eliminarOperacionTalRompeBacoFisico(id);
...
Future<void> _enviarDatosALaNube(List<Map<String, dynamic>> jsonData) async {
  ...
  try {
    ...
    /// 🔥 ENVÍO MASIVO (tipo correcto)
    final success = await operacionService.crear('rompebanco', dataParaEnviar);
    ...
    if (success) {
      ...
      for (var item in jsonData) {
        ...
        await _dbHelper.actualizarEnvioRompeBancos(localId);
      }
