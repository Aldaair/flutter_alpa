...
class DetalleSeccionScreen extends StatefulWidget {
  ...
  @override
  State<DetalleSeccionScreen> createState() => _DetalleSeccionScreenState();
}
...
class _DetalleSeccionScreenState extends State<DetalleSeccionScreen> {
  ...
  late ExportarService _exportarService;

  @override
  void initState() {
    super.initState();
    _dbHelper = DatabaseHelper();
    _exportarService = ExportarService(_dbHelper);
    _fetchOperacionData();
  }
  ...
  Future<void> _eliminarRegistrosSeleccionados() async {
    ...
    try {
      ...
      for (var id in selectedItems) {
        int result = await _dbHelper.eliminarOperacionTalLargoFisico(id);
...
Future<void> _enviarDatosALaNube(List<Map<String, dynamic>> jsonData) async {
  ...
  try {
    // 🔥 PREPARAR DATA (quitamos local_id)
    ...
    // 🔥 ENVÍO MASIVO
    final success = await operacionService.crear('tal_largo', dataParaEnviar);
    ...
    if (success) {
      ...
      for (var item in jsonData) {
        ...
        await _dbHelper.actualizarEnvio(localId);
      }
