...
class ApiServiceCategoriaEstado {
  ApiServiceCategoriaEstado({HorizontalCatalogRepository? catalogRepository})
      : _catalogRepository =
            catalogRepository ?? HorizontalCatalogRepository();
  ...
  Future<List<CategoriaEstado>> fetchCategoriasEstados(String token) async {
    try {
      final response = await http.get(
        Uri.parse(
            '${ApiConfig.baseUrl}${ApiConfig.categoriasEstadosEndpoint}'),
      ...
      if (response.statusCode == 200) {
        ...
        final categorias =
            responseData.map((d) => CategoriaEstado.fromJson(d)).toList();
