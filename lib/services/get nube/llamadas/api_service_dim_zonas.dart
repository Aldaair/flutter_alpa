...
class ApiServiceDimZonas {
  ApiServiceDimZonas({HorizontalCatalogRepository? catalogRepository})
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final zonas = decoded
            .whereType<Map>()
            .map(
              (data) => DimZona.fromJson(
                data.map((key, value) => MapEntry(key.toString(), value)),
              ),
            )
            .toList();

        await _catalogRepository.refreshDimZonas(zonas);
        return zonas;
      }
